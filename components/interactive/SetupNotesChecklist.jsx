...
const CHECKLIST_ITEMS = [
    { id: 'github', category: 'GitHub & Security', task: 'Create a GitHub account and explicitly enable Two-Factor Authentication (2FA).' },
...
export default function SetupNotesChecklist() {
    ...
    const generateMarkdown = () => {
        ...
        CHECKLIST_ITEMS.forEach(item => {
            const box = checkedItems[item.id] ? '[x]' : '[ ]'
            md += `- ${box} **[${item.category}]** ${item.task}\n`
        })
        return md
    }
    ...
    const completed = CHECKLIST_ITEMS.filter(item => checkedItems[item.id]).length
    ...
    return (
        <div className="bg-white text-slate-900 p-6 rounded-xl border border-slate-200 max-w-2xl mx-auto my-8 shadow-lg print:border-none print:shadow-none print:p-0">
            <div className="flex justify-between items-start gap-4 border-b border-slate-100 pb-4 mb-6 print:mb-4">
                <div>
                    <h2 className="text-2xl font-bold text-slate-950">SETUP-NOTES.md Beginner Checklist</h2>
                    <p className="text-xs text-slate-500 mt-1 print:hidden">
                        Track your onboarding progress. Fits on one screen or one printed page. {completed}/{CHECKLIST_ITEMS.length} done.
                    </p>
                </div>
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 text-center text-[11px] text-slate-400 print:hidden">
                Stays completely static and client-only. No backend, telemetry, or secrets.
            </div>
        </div>
    )
}
